...
### Baselines
* **`naive.c`**: Naive C implementation running on the CPU (Standard Triple-For-Loop).
* **`main.c`**: Optimized C implementation running on the CPU.
...
| :--- | :--- | :--- | :--- | :--- |
| **CPU** | 1. Naive C | `naive.c` | Standard Triple-For-Loop | ~ 0.40 |
| **CPU** | 2. Optimized C | `main.c` | Optimized C Implementation | ~ 75 |
| **CPU** | 3. NumPy (Python) | `main.py` | AVX/SIMD Vectorized BLAS | ~ 320 - 350 |
| **GPU** | 4. Shared Memory Tiled | `gemm_warp.cu` | L1 Cache / Shared Mem | ~ 637 |
| **GPU** | 5. Register Tiled | `gemm_reg.cu` | Thread Coarsening | ~ 1,864 |
| **GPU** | 6. Tensor Cores (Naive) | `wmma_gemm.cu` | Global Mem -> Tensor Cores | ~ 4,025 |
| **GPU** | 7. Tensor Cores + Shared | `wmma_shared.cu` | Global -> Shared -> Tensor Cores | ~ 4,250 |
| **GPU** | 8. cuBLAS SGEMM | `cublas_gemm.cu` | `cublasSgemm` (Standard Cores) | ~ 5,280 |
| **GPU** | 9. Software Pipelining | `wmma_pipeline.cu` | Double Buffering *(Occupancy Drop)* | ~ 3,404 |
| **GPU** | **10. Hardware Async Pipeline** | **`wmma_async.cu`** | **`cp.async` Bypassing Registers** | **~ 6,779** |
...
1. **Beating the Naive C Code (0.40 -> 75 GFLOPS):** Standard C optimizations dramatically outperform a naive triple-for-loop matrix multiplication.
2. **Beating the Optimized CPU (75 -> 350 GFLOPS):** NumPy crushes raw C code by utilizing CPU-specific SIMD (Single Instruction, Multiple Data) vector instructions like AVX-512.
3. **Beating standard CUDA Cores (637 -> 1864 GFLOPS):** Moving from shared memory to registers (`gemm_reg.cu`) ensures the math units aren't waiting on the L1 cache.
4. **Beating cuBLAS SGEMM (5280 -> 6779 GFLOPS):** The `cublasSgemm` function is strictly defined to perform Single Precision (FP32) math using standard CUDA cores. To surpass this 5.2 TFLOP limit, we pivoted to mixed-precision math via the `wmma` API. 
...
5. **The Final Leap (4250 -> 6779 GFLOPS):** Even with Tensor Cores, threads wasting time moving memory stalls performance. 
